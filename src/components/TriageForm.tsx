...
export const TriageForm: React.FC<TriageFormProps> = ({ id }) => {
  ...
  const handleFinish = () => {
    const message = `Olá! Gostaria de agendar uma consulta. Seguem meus dados de triagem:
...
*Grau de Urgência:* ${data.urgency}/5`;
  ...
  return (
    <section id={id} className="py-16 px-4 bg-white min-h-[500px]">
      <div className="max-w-md mx-auto bg-white rounded-3xl shadow-xl border border-sage-100 overflow-hidden">

        {/* Header with Progress */}
        <div className="bg-sage-50 p-6 pb-8 border-b border-sage-100">
          <h2 className="text-2xl font-serif text-sage-900 text-center mb-2">Triagem Inteligente</h2>
          <p className="text-center text-sage-600 text-xs mb-4">Responda para agilizarmos seu atendimento</p>
        ...
        <div className="p-6">
          ...
          {step === 1 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <label className="block">
                <span className="text-sage-700 font-bold text-sm">Qual seu nome?</span>
                <input
                  type="text"
                  value={data.name}
                  onChange={e => setData({ ...data, name: e.target.value })}
                  className="mt-1 block w-full px-4 py-3 bg-sage-50 border-transparent focus:border-sage-500 focus:bg-white focus:ring-0 rounded-xl transition-colors"
                  placeholder="Seu nome completo"
                />
              </label>

              <label className="block">
                <span className="text-sage-700 font-bold text-sm">Sua idade?</span>
                <input
                  type="number"
                  value={data.age}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <p className="text-sage-700 font-bold text-sm mb-2">Qual o motivo principal?</p>
              ...
              <div className="pt-4 flex gap-3">
                <Button variant="outline" onClick={handleBack} className="px-6 text-sage-600 border-sage-300 hover:bg-sage-100">
                  Voltar
                </Button>
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <p className="text-sage-700 font-bold text-sm text-center">Já fez terapia antes?</p>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 py-6 rounded-xl font-bold transition-all border-2 ${data.previousTherapy === 'Sim'
                    ...
                    : 'border-sage-100 text-sage-400 hover:border-sage-300'
                    }`}
                >
                  SIM
                </button>
          ...
          {step === 4 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <label className="block">
                <span className="text-sage-700 font-bold text-sm">Melhor horário para contato?</span>
                <select
                  value={data.bestTime}
                  onChange={e => setData({ ...data, bestTime: e.target.value })}
                  className="mt-1 block w-full px-4 py-3 bg-sage-50 border-r-8 border-transparent focus:border-sage-500 focus:bg-white focus:ring-0 rounded-xl"
                >
                  {TIME_SLOTS.map(slot => (
              ...
              <div className="block">
                <span className="text-sage-700 font-bold text-sm flex justify-between">
                  <span>Grau de Urgência</span>
                  <span className="bg-sage-100 text-sage-800 px-2 py-0.5 rounded text-xs">{data.urgency}/5</span>
                </span>
                <input
                  type="range"
                  min="1"
                  ...
                  className="w-full mt-3 h-2 bg-sage-200 rounded-lg appearance-none cursor-pointer accent-sage-600"
                />
              ...
              <div className="pt-4 flex flex-col gap-3">
                ...
                <button
                  onClick={handleBack}
                  className="text-sage-500 hover:text-sage-800 text-sm font-medium transition-colors py-2"
                >
